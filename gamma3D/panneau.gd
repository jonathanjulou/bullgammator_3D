...
func _draw():
	...
	for ligne in range(16):
		...
		for i in range(8):
			...
			if i/2 != i/2.0 and code[i/2] != 0:
				#connecte(center, code[i/2], eq_classes[code[i/2]][-2])
				to_connecte.append([center, code[i/2], eq_classes[code[i/2]][-2]])
			else:
	...
	if current_color != 0:
		if current_color < 16:
			draw_circle(eq_classes[current_color][0], 20, Color(1,1,1))
		
	for e in to_connecte:
		connecte(e[0], e[1], e[2])
...
func connecte(point, classe, last):
	var lastpoint = point
	for t in range(20):
		#draw_line ( point, eq_classes[classe][-2], color_map(classe), 6.0, true )
		point = bezier_bump(point, last, t/20.0)
		draw_line(lastpoint, point, color_map(classe), 6.0, true )
		lastpoint = point
	draw_line(lastpoint, last, color_map(classe), 6.0, true )
